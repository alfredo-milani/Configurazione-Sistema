...
function manage_bumblebee {
    ...
    sudo tee -a <<< "blacklist nouveau" /etc/modprobe.d/nouveau-blacklist.conf 1> $null
    ...
    cat << EOF
NOTA: è consigliato usare primusrun rispetto ad optirun perchè più veloce
NOTA: per testare bumblebee: $ primusrun/optirun -vv glxgears
Per tools di benchmarking visitare questo sito: 'http://www.geeks3d.com/gputest/download/'
Per utilizzare nvidia-settings usare il comando: 'optirun nvidia-settings -c :8'
EOF
